...
export default function PlatformEngagementDonut({ summary }: PlatformEngagementDonutProps) {
  ...
  const total = summary?.total_engagement_actions || likes + comments + postViews + profileViews
      ...
      <div className="h-[220px] w-full relative z-10 flex items-center justify-center">
        {hasData ? (
                    ...
                    return (
                      <div className="bg-slate-900/95 border border-slate-700/80 rounded-xl px-4 py-2.5 shadow-2xl backdrop-blur-md">
                        ...
                        <p className="text-sm font-extrabold text-indigo-400 mt-0.5">
                          {item.value} <span className="text-slate-400 text-xs">({pct}%)</span>
                        </p>
                      </div>
                    )
                  }
          ...
          <div className="flex flex-col items-center justify-center h-full border border-dashed border-slate-800 rounded-xl w-full">
            ...
            <p className="text-slate-500 text-xs italic">No engagement data recorded yet.</p>
          </div>
        )}
      </div>
        ...
        {data.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[9px] text-slate-500 font-semibold">{pct}% share</span>
            </div>
          )
        })}
      </div>
