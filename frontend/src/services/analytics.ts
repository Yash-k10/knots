...
export const analyticsService = {
  ...
  getPlatformEngagementSummary:
    async (): Promise<PlatformEngagementSummary> => {
      return apiRequest<PlatformEngagementSummary>(
        "/analytics/platform/engagement-summary",
      );
    },

  getProfileViews: async (days: number = 7): Promise<ProfileViewsResponse> => {
    return apiRequest<ProfileViewsResponse>(
      `/analytics/profile/views?days=${days}`,
    );
  },
  ...
  getTrendingPosts: async (
    ...
    return apiRequest<TrendingPost[]>(
      `/analytics/trending-posts?limit=${limit}&days=${days}`,
    );
  },
