...
export const aiService = {
  async getConnectionSuggestions(limit = 6): Promise<ConnectionSuggestion[]> {
    return apiRequest<ConnectionSuggestion[]>(`/ai/connection-suggestions?limit=${limit}`)
  },

  async getJobRecommendations(limit = 6): Promise<JobRecommendation[]> {
    return apiRequest<JobRecommendation[]>(`/ai/job-recommendations?limit=${limit}`)
  },

  async getContentRecommendations(limit = 6): Promise<ContentRecommendation[]> {
    return apiRequest<ContentRecommendation[]>(`/ai/content-recommendations?limit=${limit}`)
  },
  ...
  async generateRoadmap(targetRole: string, currentSkills: string[]): Promise<CareerRoadmapResult> {
