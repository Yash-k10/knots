...
class RoleRequired:
    ...
    def __call__(self, current_user: User = Depends(get_current_user)) -> User:
        ...
        if current_user.role.name not in self.allowed_roles:
            raise AuthorizationError(f"Role not authorized. Required one of: {self.allowed_roles}")
        return current_user
...
class PermissionRequired:
    ...
    def __call__(self, current_user: User = Depends(get_current_user)) -> User:
        ...
        if self.required_permission not in permissions:
            raise AuthorizationError(f"Permission denied: missing {self.required_permission}")
        return current_user
