...
class PostService:
    ...
    async def like_post(self, post_id: int, user_id: int) -> Like:
        """Like a post idempotently. If already liked, returns existing like."""
        ...
        if post.author_id != user_id:
            try:
                from app.notifications.services.notification import (
                    NotificationService,
                )
                ...
                liker_name = (
                    f"{liker_prof.first_name} {liker_prof.last_name}".strip()
                    if (
                        liker_prof
                        and hasattr(liker_prof, "first_name")
                        and liker_prof.first_name
                    )
                    else "A member"
                )
    ...
    async def unlike_post(self, post_id: int, user_id: int) -> None:
        """Remove a like from a post idempotently."""
        existing = await self.like_repo.get_by_post_and_user(post_id, user_id)
        if existing:
